...
class PhraseDALabeller(object):
	...
	def find_phrase_da_labels(self, phrase_tokens):
		...
		user_placeholder_phrases = dict(
			(username, tuple(USERNAME_PLACEHOLDER_TOKEN if token == username else token for token in phrase_tokens)) for
			username in self.usernames)
...
#	print(da_labels, file=sys.stderr)
	da_targets = DialogueActTargets(da_labels, phrase_da_labeller.usernames)
...
#		"dialog-acts" : da_label_targets
#	}
	_mq.publish(exchange=env_exchange_name, routing_key=settings["dialogue_acts"] + ".{}".format(participant), body=da_label_targets)
